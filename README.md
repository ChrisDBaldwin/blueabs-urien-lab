# BlueAbs Urien Lab

A Lua training script for **Urien** in **Street Fighter III: 3rd Strike** on [FBNeo](https://github.com/finalburnneo/FBNeo).

Learn Urien combos step by step — from basic confirms through tackle loops to Aegis unblockable setups. Record your own combos and the script turns them into exercises you can practice.

## Features

- **Guided exercises** — Combo, unblockable, sequence, and parry exercises with progression tracking (3 clears = mastered)
- **Record your own** — Press Start to capture a combo, and it becomes a playable exercise with auto-detected notation
- **Per-opponent save states** — Save and load match states for each of the 19 characters
- **Live combo tracker** — See move names in real time during free practice
- **Charge meters** — Live charge bars with decay timers (b-f/d-u or 4-6/2-8 depending on notation mode)
- **Training mode always on** — Infinite timer, full meter, stun cleared, HP recovers after combos drop
- **Aegis multi-combo support** — Exercises that span multiple combo sequences (counter resets between parts)
- **Dual notation** — Street Fighter (`b~f+MK`) or numpad (`[4]6MK`)
- **Opponent tagging** — Tag exercises per matchup so only relevant combos show
- **Editable exercise files** — Rename, tweak hints, or add exercises in a text file, then reload with Alt+9

## Requirements

- [FBNeo](https://github.com/finalburnneo/FBNeo) emulator (with Lua scripting support)
- SF3: 3rd Strike ROM (`sfiii3`)

## Installation

1. Clone or download this repository
2. Place `urien_lab.lua` somewhere accessible to FBNeo (e.g., next to the emulator or in a scripts folder)
3. Launch FBNeo and load the `sfiii3` ROM
4. Open the Lua console: **Game > Lua Scripting**
5. Load `urien_lab.lua`

The script will auto-load the first available save state and show the character select grid.

## How to Use

### Quick Load Menu
1. Press **Coin** to open the main menu
2. Select **Change Opponent** to pick a matchup, or load a saved state from the character grid

### Save a Match State (once per opponent)

1. Start a match as **Urien** vs any opponent
2. Press **Start** to open the character select overlay
3. Navigate to the opponent with **D-Pad**
4. Press **Fierce** to save — reload any time with **Jab**

### Practice Menu

1. In a match press **Coin** to open the main menu
2. Pick **Tutorial** or **Combo Trials**
3. Browse with **Up/Down**, switch tabs with **Left/Right**
4. Press **Jab** to start an exercise
5. Perform the combo — the HUD tracks your progress
6. Clear it 3 times to master it

### Record Your Own

1. Press **Start** to start recording
2. Do the combo
3. Press **Start** again — pick a category, and the exercise appears in your menu
4. Rename it in `urien_lab_custom.txt` and press **Alt+9** to reload

### Controls

#### Character Select

| Input | Action |
|-------|--------|
| D-Pad | Navigate character grid |
| Start | Toggle overlay (hide to play, show to save/load) |
| Jab (Weak Punch) | Load saved state for selected opponent |
| Fierce (Strong Punch) | Save current match state for selected opponent |

#### Training Mode

| Input | Action |
|-------|--------|
| Coin | Open/close main menu (Tutorial, Combos, Record, Change Opponent) |
| Start | Toggle recording (closes exercise menu if open) |
| Up / Down | Navigate exercise list |
| Left / Right | Switch tabs (Tutorial / All / Character / Opponent) |
| Jab (Weak Punch) | Select exercise / Change opponent |
| Strong (Medium Punch) | Stop exercise / Close menu |
| Fierce (Strong Punch) | Cycle sort mode (Category / Difficulty / Name) |
| Roundhouse (Strong Kick) | Delete exercise (press twice to confirm) |
| Weak Kick | Toggle exercise side (L/R) |
| Medium Kick | Tag/untag current opponent on highlighted exercise |

#### Hotkeys

| Input | Action |
|-------|--------|
| Alt+2 | Toggle notation (SF / Numpad) |
| Alt+3 | Reset current exercise progress |
| Alt+4 | Toggle debug display |
| Alt+5 | Toggle exercise menu directly |
| Alt+9 | Reload exercises from file |

## Exercise File Format

Exercises live in `urien_lab_exercises.txt`. Each exercise is a `---`-delimited block:

```
---
ID:c_01
NAME:cr.HP > L.Tackle
NOTATION_SF:d+HP, b~f+LK
NOTATION_NP:2HP, [4]6LK
DIFFICULTY:2
CATEGORY:combo
CHARS:Ken,Yun
SETUP:0100,0180,A0,A0,full,1,0,stand,0
SEQ:cr.HP,H,A00180018|L.Tackle,H,S003a003a
SUCCESS:2
RESETOK:1
HINT:d+HP -> b~f+LK
---
```

Most fields are auto-generated by the recorder. To customize, edit `NAME`, `HINT`, `CATEGORY` (`combo`/`unblockable`/`sequence`/`parry`), `CHARS` (opponent filter), or `DIFFICULTY` (1–5). Add `RESETOK:1` for Aegis setups where the combo counter resets mid-sequence.

## Architecture

Single Lua file (~4900 lines), no dependencies. See `CLAUDE.md` for the full section map and technical details.

Two state machines drive the script:
- **App level** — character select vs training mode
- **Exercise level** — idle → active → success/fail

## Acknowledgments

Built on work from the SF3:3S Lua community:

- [**3rd_training_lua**](https://github.com/Grouflon/3rd_training_lua) by Grouflon — memory map, drawing conventions, resource management
- [**SF3 3rd Strike Trial Script**](https://ameblo.jp/3fv/entry-12747992757.html) by crystal_cube99 — action ID format and hit detection approach

## Contributing

### Add exercises (easiest way to help)

1. Record a combo with **Start**
2. Test that it detects reliably
3. Copy your combo from `urien_lab_custom.txt` and paste it into `urien_lab_exercises.txt`
4. Clean up the name and hint in `urien_lab_exercises.txt`
5. Submit a PR

### Add missing moves

If the tracker shows `spc?XXXX` or `atk?XXXX`, that move isn't in the database yet.

1. Turn on debug (Alt+4) and do the move
2. Note the action string (e.g., `S003e003e`)
3. Add it to the `MOVES` table in `urien_lab.lua`
4. Submit a PR

### Code changes

1. Fork and branch
2. Edit `urien_lab.lua` (single file, no `require`)
3. Test in FBNeo — check exercises and save/load still work
4. Open a PR

See `CLAUDE.md` for technical conventions (memory write timing, action ID format, color format).

## License

Provided as-is for the fighting game community.
